...
def create_db(config)->bool:
    ...
    if cnt[0] == 0:
        # create db
        SQL_QUERY = "CREATE DATABASE [{}]".format(config['DBGP'])
        cursor.execute(SQL_QUERY)
        cnxn.close()
        # create tables
        create_table_gpresult(config)
        return True
    return False

def create_table_gpresult(config):
    drv = pyodbc.drivers()
    DRIVER_NAME=drv[3]
    SQL_STR="Driver={"+DRIVER_NAME+"};SERVER="+config['SERVER']+';DATABASE='+config['DBGP']+';UID='+config['UID']+';PWD='+config['PWD']
    cnxn = pyodbc.connect(SQL_STR, autocommit=True)
    cursor = cnxn.cursor()
    SQL_QUERY="""CREATE TABLE [dbo].[gpresult](
...
def get_status(config,function_id:int)->mathfunc_pb2.State:
    ...
    result=cursor.execute(
        """SELECT [scheduled],[ready] FROM [dbo].[gpresult]
            WHERE [FunctionId]=?
    ...
    if result is None:
        ...
        assert not (result[0] and result[1]),"in db_gp, get_status: scheduled and resultready are exclusive." # entry in db is wrong
    cnxn.close()
    return state
